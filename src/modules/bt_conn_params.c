...
LOG_MODULE_REGISTER(MODULE, CONFIG_LIBREWATCH_BT_CONN_PARAMS_LOG_LEVEL);
...
static void update_conn_params(void)
{
    ...
    struct bt_le_conn_param param = {
        .interval_min = CONFIG_LIBREWATCH_BT_CONN_PARAMS_INT_MIN,
        .interval_max = CONFIG_LIBREWATCH_BT_CONN_PARAMS_INT_MAX,
        .latency = CONFIG_LIBREWATCH_BT_CONN_PARAMS_LATENCY,
        .timeout = CONFIG_LIBREWATCH_BT_CONN_PARAMS_TIMEOUT,
    };
...
static bool app_event_handler(const struct app_event_header *aeh)
{
    if (is_ble_peer_event(aeh)) {
        ...
        if (event->state == PEER_STATE_CONNECTED) {
            conn = event->id;
#if !IS_ENABLED(CONFIG_LIBREWATCH_DISCOVERY)
            update_conn_params();
#endif // IS_ENABLED(CONFIG_LIBREWATCH_DISCOVERY)
